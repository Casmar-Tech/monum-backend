 ...
 * @param {Object} place - The place for which to generate audio guides
 * @param {number} numGuides - Number of guides to generate
 * @returns {Promise<Array>} - Array of titles and beginnings
 */
async function generateTitlesAndBeginnings(place, numGuides) {
	try {
		...
		try {
			...
			if (jsonMatch) {
				...
				titlesAndBeginnings = JSON.parse(response);
			}
		} catch (parseError) {
 ...
 * @param {Object} place - The place for which the audio guide is generated
 * @param {Object} titleAndBeginning - Object containing title and beginning of the guide
 * @returns {Promise<Object>} - A complete audio guide
 */
async function generateFullAudioguide(place, titleAndBeginning) {
	try {
		const name = place.name || '';
		...
		try {
			...
			if (jsonMatch) {
				...
				try {
					...
					// If that fails, try a more aggressive approach: Use eval (with safety measures)
					console.log(
				...
				audioguide = JSON.parse(cleanedFullResponse);
			}
 ...
 * @param {Object} place - The place to generate guides for
 * @param {number} numGuides - Number of guides to generate
 * @returns {Promise<Array>} - Array of generated audio guides
 */
async function generateAudioguides(place, numGuides) {
	const guides = [];
...
async function processPlaces() {
	try {
		...
		const places = JSON.parse(inputData);
		...
		for (let i = 0; i < places.length; i++) {
			...
			// Check if we've already generated guides for this place
			...
			// Generate guides for this place using the two-step process
			const guides = await generateAudioguides(place, numGuides);

			if (guides && guides.length > 0) {
				// Add new guides to the array
				allAudioguides = [...allAudioguides, ...guides];

				// Save partial results with the correct structure
				fs.writeFileSync(
					outputFile,
				...
				console.log(
					`Generated ${guides.length} audio guides for ${place.name}.`,
				);
			} else {
				console.error(`Error generating audio guides for ${place.name}.`);
			}

			// Wait a moment between places
			if (i < places.length - 1) {
				console.log('Waiting before the next place...');
